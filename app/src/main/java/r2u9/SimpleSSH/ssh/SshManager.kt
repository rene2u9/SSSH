...
class SshSession(
    ...
    fun close() {
        try {
            ...
            Logger.e(TAG, "Failed to close session", e)
        }
    }
    ...
    fun resizePty(cols: Int, rows: Int) {
        try {
            Logger.d(TAG, "Resizing PTY to ${cols}x${rows}")
            shell.changeWindowDimensions(cols, rows, 0, 0)
            Logger.d(TAG, "PTY resize successful")
        } catch (e: Exception) {
            Logger.e(TAG, "Failed to resize PTY: ${e.message}", e)
        }
    }
...
object SshManager {
    ...
    suspend fun connect(connection: SshConnection): Result<SshSession> = withContext(Dispatchers.IO) {
        try {
            Logger.d(TAG, "Creating SSH client...")
            val client = SSHClient()
            client.addHostKeyVerifier(PromiscuousVerifier())

            Logger.d(TAG, "Connecting to ${connection.host}:${connection.port}...")
            client.connect(connection.host, connection.port)
            Logger.d(TAG, "Connected to server, authenticating...")

            when (connection.authType) {
                AuthType.PASSWORD -> {
                    Logger.d(TAG, "Using password authentication for user: ${connection.username}")
                    client.authPassword(connection.username, connection.password ?: "")
                }
                AuthType.KEY -> {
                    Logger.d(TAG, "Using key authentication for user: ${connection.username}")
            ...
            Logger.d(TAG, "Starting session and allocating PTY...")
            val session = client.startSession()
            session.allocatePTY("xterm-256color", 80, 24, 0, 0, emptyMap())
            val shell = session.startShell()
            Logger.d(TAG, "Shell started successfully")
            ...
            Logger.d(TAG, "SSH session created with id: ${sshSession.id}")
            Result.success(sshSession)
        } catch (e: Exception) {
            Logger.e(TAG, "SSH connection failed: ${e.message}", e)
            Result.failure(e)
        }
    }
