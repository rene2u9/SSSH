...
object WcWidth {
    ...
    // Combining characters (width 0) - stored as [start, end] pairs
    private val COMBINING = intArrayOf(
        ...
        0x1122C, 0x11237, 0x1123E, 0x1123E, 0x112DF, 0x112EA, 0x11300, 0x11303,
        ...
        0x11EF3, 0x11EF6, 0x16AF0, 0x16AF4, 0x16B30, 0x16B36, 0x16F4F, 0x16F4F,
        0x16F51, 0x16F87, 0x16F8F, 0x16F92, 0x16FE4, 0x16FE4, 0x1BC9D, 0x1BC9E,
    ...
    // Wide characters (width 2) - stored as [start, end] pairs
    private val WIDE = intArrayOf(
        ...
        0x3000, 0x3000, 0x3004, 0x3004, 0x3008, 0x3011, 0x3014, 0x301F,
        0x3030, 0x3030, 0x303E, 0x303E, 0x3041, 0x3096, 0x309B, 0x30FF,
        0x3105, 0x312F, 0x3131, 0x318E, 0x3190, 0x31E3, 0x31F0, 0x321E,
        0x3220, 0x3247, 0x3250, 0x4DBF, 0x4E00, 0x9FFF, 0xA000, 0xA48C,
        0xA490, 0xA4C6, 0xA960, 0xA97F, 0xAC00, 0xD7A3, 0xF900, 0xFAFF,
        ...
        0x1F947, 0x1F9FF, 0x1FA00, 0x1FA53, 0x1FA60, 0x1FA6D, 0x1FA70, 0x1FA74,
    ...
    private fun bisearch(ucs: Int, table: IntArray): Boolean {
        ...
        while (max >= min) {
            val mid = (min + max) / 2
            when {
                ucs > table[2 * mid + 1] -> min = mid + 1
                ucs < table[2 * mid] -> max = mid - 1
                else -> return true
            }
        }
        return false
     ...
     * @return -1 for non-printable, 0 for combining, 1 for normal, 2 for wide
     */
    fun width(codePoint: Int): Int = when {
        codePoint == 0 -> 0
        codePoint < 32 || codePoint in 0x7F..0x9F -> -1
        bisearch(codePoint, COMBINING) -> 0
        bisearch(codePoint, WIDE) -> 2
        else -> 1
    }
}
